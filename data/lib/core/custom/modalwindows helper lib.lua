...
function MT:getButtonCount()
	return #self.buttons
end

function MT:getChoiceCount()
	return #self.choices
end
...
function structWindow(title, message, choices) window = ModalWindow{title = title, message = message} for i = 1, choices do window:addChoice(i) end window:addButton("Close") window:setDefaultEscapeButton("Close") return window end
function MT:getTitle()
	return self.title
end

function MT:getMessage()
	return self.message
end
